...
// 2opt implementado com a estratégia de first improvement
...
void cruzamentoBASEDPOS(int *pai1, int *pai2, int *f1, int *f2)
{
    ...
    int n = 10;
    int pos[] = {10, 110, 210, 310, 410, 510, 610, 710, 810, 910};
    // for (int i = 0; i < n; i++)
    // {
    //     int aux = rand() % SIZE;
    //     while (verificarConjunto(pos, aux, n))
    //     {
    //         aux = rand() % SIZE;
    //     }
    //     pos[i] = aux;
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        auto it1 = find(pai1, pai1 + SIZE, f1[pos[i]]);
        int index = distance(pai1, it1);
        f1[index] = pai1[pos[i]];

        auto it2 = find(pai2, pai2 + SIZE, f2[pos[i]]);
        int index2 = distance(pai2, it2);
        f2[pos[i]] = pai1[pos[i]];
        f2[index2] = pai2[pos[i]];
    }
}
...
int main()
{
    ...
    while (geracoes < MAXGERACOES and contador < PARADA)
    {
        ...
        cruzamentoBASEDPOS(population[idx_pai1], population[idx_pai2], filho1, filho2);
